import MARKERDATA from "../js/markerDataBlue.js";

var scene, camera, renderer;

var arToolkitSource, arToolkitContext;

var roots = [];

let timer = 100;
let delay = true;

const studentName = parseURL();

let studentMarker;

function parseURL() {
  const url = window.location.href;
  const projName = url.substring(url.lastIndexOf("/") + 1);
  return projName.split(".")[0];
}

function init() {
  scene = new THREE.Scene();

  let ambientLight = new THREE.AmbientLight(0xcccccc, 0.5);
  scene.add(ambientLight);

  camera = new THREE.Camera();
  scene.add(camera);

  renderer = new THREE.WebGLRenderer({
    antialias: true,
    alpha: true
  });

  arToolkitSource = new THREEx.ArToolkitSource({
    sourceType: "webcam"
  });

  function onResize() {
    arToolkitSource.onResizeElement();
    arToolkitSource.copySizeTo(renderer.domElement);
    // arToolkitSource.copyElementSizeTo(renderer.domElement);
    if (arToolkitContext.arController !== null) {
      arToolkitSource.copyElementSizeTo(arToolkitContext.arController.canvas);
    }
  }

  arToolkitSource.init(function onReady() {
    onResize();
  });

  window.addEventListener("resize", function() {
    onResize();
  });

  arToolkitContext = new THREEx.ArToolkitContext({
    cameraParametersUrl: "../data/camera_para.dat",
    detectionMode: "mono",
    maxDetectionRate: 30,
    patternRatio: 0.5
  });

  arToolkitContext.init(function onCompleted() {
    camera.projectionMatrix.copy(arToolkitContext.getProjectionMatrix());
  });

  MARKERDATA.forEach(marker => {
    let markerRoot = new THREE.Group();
    markerRoot.url = marker.url;
    if (marker.url == studentName) {
      studentMarker = roots.length;
    }
    roots.push(markerRoot);
    scene.add(markerRoot);
    // console.log(marker.pattern);
    new THREEx.ArMarkerControls(arToolkitContext, markerRoot, {
      type: "pattern",
      patternUrl: marker.pattern
    });
  });

  animate();
}

function animate() {
  requestAnimationFrame(animate);

  if (timer > 0 && delay) {
    timer--;
    if (timer <= 0) {
      delay = false;
    }
    return;
  }

  if (arToolkitSource.ready !== false) {
    arToolkitContext.update(arToolkitSource.domElement);
  }

  if (roots[studentMarker].visible) {
    if (timer > 0) {
      timer = 0;
    }
  } else {
    if (timer < 100) {
      timer++;
    } else {
      roots.forEach(marker => {
        if (marker.visible) {
          updateShown(marker.url);
        }
      });
      timer++;
      if (timer > 150) {
        location.replace("http://localhost:8080");
      }
    }
  }
}

function updateShown(value) {
  document.getElementById("isSeen").innerHTML = value;
}

init();
